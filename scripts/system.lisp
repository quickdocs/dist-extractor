...
(defun asdf-system-metadata (system-name)
  (let* ((system (let ((*standard-output* (make-broadcast-stream)))
    ...
    `(("name" . ,(name (asdf:component-name system)))
      ...
      ("authors" . ,(authors (asdf:system-author system)))
      ("maintainers" . ,(maintainers (asdf:system-maintainer system)))
      ...
      ("defsystem_depends_on" . ,(depends-on (asdf:system-defsystem-depends-on system) system-dir))
      ("depends_on" . ,(depends-on (asdf:system-depends-on system) system-dir))
      ("weakly_depends_on" . ,(depends-on (asdf:system-weakly-depends-on system) system-dir)))))
